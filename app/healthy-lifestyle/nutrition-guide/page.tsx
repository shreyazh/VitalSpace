...
export default function NutritionGuide() {
  ...
  const generateNutritionPlan = () => {
    ...
    if (gender && ageNum && weightNum) {
      let baseCaloricNeeds = gender === 'male' ? 
        (10 * weightNum + 6.25 * 170 - 5 * ageNum + 5) :
        (10 * weightNum + 6.25 * 170 - 5 * ageNum - 161)

      let activityFactor = 1.2 // Sedentary
      let dailyCalories = Math.round(baseCaloricNeeds * activityFactor)

      let protein = Math.round(weightNum * 0.8) // 0.8g per kg of body weight
      let fat = Math.round((dailyCalories * 0.3) / 9) // 30% of calories from fat
